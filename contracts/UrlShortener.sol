...
contract UrlShortener {
    ...
    constructor() public {
        // initializes admin as the contract creator(has special priviledges)
    ...
    function hashUrl(string memory _url) internal pure returns (bytes memory) {
        ...
        uint256 shiftBy = 100; // shift bits by
    ...
    function shortenUrl(string memory _url, bool _paid) public payable {
        ...
        if (!table[shortenedHash].exists) {
            // check is shortened url is already in table
            table[shortenedHash] = urlStruct(
                msg.sender,
                _url,
                true,
                _paid || initialPaid
            ); // add url to table of mappings
            ...
            if (shortenedUrls[msg.sender].length <= 1) {
                // check if it is the first time for a person to shorten an array
    ...
    function getUrl(bytes memory _shortUrl)
        public
        view
        returns (string memory)
    {
        ...
        if (shortenedUrl.exists) {
            // check if shortened url exists in table
    ...
    function destroy() public {
        // delete a smart contract from the blockchain
        if (msg.sender == admin) {
            // remaining ether on contract sent to a admin.
